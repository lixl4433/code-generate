...
import com.code.generate.config.converts.MySqlTypeConvert;
import com.code.generate.config.converts.OracleTypeConvert;
import com.code.generate.config.converts.PostgreSqlTypeConvert;
...
public class DataSourceConfig {
    ...
    public  List<String> getTableNames() {
        ...
        try {
            ...
            while (rs.next()) {
                tableNames.add(rs.getString(3)+"=>"+rs.getString(5));
            }
            rs.close();
...
//        System.out.println(getTableNames().toString());
        ...
        for(int i = 0; i < tables.length; ++i){
            ...
            String[] table_info = tables[i].split("=>");
            tableInfo.setName(table_info[0].replace("zzlc$", ""));
            tableInfo.setComment(table_info.length >1 ? table_info[1] : "");
            List<TableField> fieldList = this.getTbfrFields("zzlc$"+tableInfo.getName());
